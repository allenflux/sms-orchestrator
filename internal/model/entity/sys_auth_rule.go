...
type SysAuthRule struct {
	...
	MenuType   int         `json:"menuType"   orm:"menu_type"   ` // 类型 0目录 1菜单 2按钮
	Weigh      int         `json:"weigh"      orm:"weigh"       ` // 权重
	IsHide     int         `json:"isHide"     orm:"is_hide"     ` // 显示状态
	Path       string      `json:"path"       orm:"path"        ` // 路由地址
	Component  string      `json:"component"  orm:"component"   ` // 组件路径
	IsLink     int         `json:"isLink"     orm:"is_link"     ` // 是否外链 1是 0否
	ModuleType string      `json:"moduleType" orm:"module_type" ` // 所属模块
	ModelId    int         `json:"modelId"    orm:"model_id"    ` // 模型ID
	IsIframe   int         `json:"isIframe"   orm:"is_iframe"   ` // 是否内嵌iframe
	IsCached   int         `json:"isCached"   orm:"is_cached"   ` // 是否缓存
	Redirect   string      `json:"redirect"   orm:"redirect"    ` // 路由重定向地址
	IsAffix    int         `json:"isAffix"    orm:"is_affix"    ` // 是否固定
